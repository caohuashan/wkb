        ...
        <div class="result-wrap">
            ...
            <div class="result-content">
                <div class="short-wrap">
					<?php if(isset($_privileges['system']['base']) || $_roleid==-1) { ?>
					<a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?file=config&action=base"><i class="icon-font">&#xe017;</i>基本配置</a>
					<?php }?>
					<?php if(isset($_privileges['system']['safety']) || $_roleid==-1) { ?>
					<a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?file=config&action=safety"><i class="icon-font">&#xe057;</i>安全验证</a>
					<?php }?>
                	<?php if(Module::isModuleInstalled('wechat') && (isset($_privileges['wechat']['config']) || $_roleid==-1)) { ?>
                    <a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=wechat&file=config&action=config"><i class="icon-font">&#xe018;</i>微信接口配置</a>
                    <?php }?>
                    <?php if(Module::isModuleInstalled('member') && (isset($_privileges['member']) || $_roleid==-1)) { ?>
                    <a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=member&file=member&action=member"><i class="icon-font">&#xe014;</i>会员管理</a>
                    <a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=member&file=member&action=level"><i class="icon-font">&#xe031;</i>会员等级管理</a>
                    <?php }?>
                    <?php if(Module::isModuleInstalled('pc') && (isset($_privileges['pc']) || $_roleid==-1)) { ?>
					<a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=pc&file=config&action=base"><i class="icon-font">&#xe018;</i>网站配置</a>
                    <a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=pc&file=config&action=template"><i class="icon-font">&#xe053;</i>模板风格</a>
                    <?php }?>
                </div>
            </div>
        </div>
        <div class="result-wrap">
            ...
            <div class="result-content">
                <ul class="sys-info-list">
                    ...
                    <li>
                        <label class="res-lab">运行环境：</label><span class="res-info"><?php echo system_software();?></span>
                    </li>
                    <li>
                    ...
                    <li>
                        <label class="res-lab">服务器域名/IP：</label><span class="res-info"><?php echo isset($_SERVER["SERVER_NAME"])?$_SERVER["SERVER_NAME"]:'';?> [ <?php echo gethostbyname($_SERVER["SERVER_NAME"]);?> ]</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="result-wrap" style="clear:both">
            ...
            <div class="result-content">
                <ul class="sys-info-list">
                    ...
                    <li>
                        <label class="res-lab">内核版本：</label><span class="res-info">Version <?php echo defined('PHPWECHAT_VERSION')?PHPWECHAT_VERSION:'{__PHPWECHAT_VERSION__}';?><?php echo defined('PHPWECHAT_RELEASE')?PHPWECHAT_RELEASE:'{__PHPWECHAT_RELEASE__}';?> </span>
						
                    </li>
                    <li>
                        <label class="res-lab">在线更新：</label><span class="res-info"><a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?file=index&action=update" onClick="if(!confirm('建议您在在线更新前备份您的程序数据和数据库，确认继续？')){return false;}" class="updatebtn">在线更新</a></span>
                    </li>
                </ul>
            </div>
        </div>
